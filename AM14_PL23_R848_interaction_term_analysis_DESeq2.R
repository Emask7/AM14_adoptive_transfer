# Wrote code to try using interaction terms. I don't think using an interaction 
# term is necessary, so I commented out this code so I don't use it, but it is
# still available in case I want to use it in the future
...
#                                 design = ~Cohort + Stim + Drug + Stim:Drug)
#   
#   dds
#   model.matrix(~Cohort + Stim + Drug + Stim:Drug, data = coldata)
#   dds$Stim <- relevel(dds$Stim, ref = "R848")
#   dds$Drug <- relevel(dds$Drug, ref = "Control")
...
#     df <- as.data.frame(colData(dds)[, c("Drug", "Stim", "Cohort")])
#     
...
#     vsd <- vst(dds)
#     
#     pcaData <- plotPCA(vsd, intgroup=c("Stim", "Drug", "Cohort"), returnData=TRUE)
#     percentVar <- round(100 * attr(pcaData, "percentVar"))
#     
#     png(filename = "Output/PCA Plot - Before Batch Correction.png", 
#         width = 1500, height = 1500, units = "px", pointsize = 10, res = 200, 
#         bg = "white", family = "", type = "windows", symbolfamily="default")
#     ggplot(pcaData, aes(PC1, PC2, color=Stim:Drug, shape=Cohort)) +
#       geom_point(size=3) +
#       xlab(paste0("PC1: ",percentVar[1],"% variance")) +
#       ylab(paste0("PC2: ",percentVar[2],"% variance")) +
#       # geom_text(label=coldata$Mouse_Number, nudge_x=2, nudge_y=1, check_overlap=F) +
#       coord_fixed() +
...
#     nrow(subset(res, res$padj <= 0.05 & res$log2FoldChange >= 1))
#     nrow(subset(res, res$padj <= 0.05 & res$log2FoldChange <= -1))
#     
#     
